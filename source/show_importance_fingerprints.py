...
def View(df):
    ...
    s += (
        ...
        + (
            df.to_html(formatters={"Image": image_formatter}, escape=False) + css
        ).replace("\n", "\\")
        + "';"
    )
    s += "</script>"
    ...
    for tmp, item in enumerate(dir_fl_names):
        try:
            ...
            fp_bit = AllChem.GetMorganFingerprintAsBitVect(
                suppl, int(2), nBits=int(bit_length)
            )
...
molArr, names, morganArr, retArr, bitInfo_arr, homo, homo1, diff = morgan_bits(
    dir="../data/sdf/DB3/", bit_length=1024
)
