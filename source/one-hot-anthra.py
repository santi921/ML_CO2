...
def get_one_hot_data():
    df_anthra= pd.read_hdf("../data/anthra/compiled.h5")
    anthra_smiles = df_anthra["smiles"]
    homo_anthra = df_anthra["homo"]
    homo1_anthra = df_anthra['homo1']
    str_anthra = []
    homo = []
    homo1 = []
    succ = 0
                 
    for ind, i in enumerate(anthra_smiles):
        ...
        str_anthra.append(frag_list_temp)

    
    list_one_hot = []
    fail = 0

    for ind, partition in enumerate(str_anthra):

        list_groups = []
        list_temp = [0 for i in range(29)]
        for ind2, split in enumerate(partition):
            add = True
            ...
            if split == "O=[N+]([O-])":
                ...
                print(split)
        if add:
            homo.append(homo_anthra[ind])
            homo1.append(homo1_anthra[ind])
            list_one_hot.append(list_temp)

    print(str(fail) + "/" + str(len(str_anthra)))
    homo = np.array(homo)
...
class optimizer_genetic(object):
    ...
    def train_models(self):
        ...
        self.homo1_model = KernelRidge(alpha=1e-3, kernel='laplacian')
        ...
        self.homo_model = XGBRegressor(eta = 0.3, max_depth = 6, n_estimators=300)
    ...
    def enrich_pop(self, gens=2, longitudnal_save=False):
        ...
        total_loss = 0
        successful_mol_count = 0
        write_to_file = str(self.start_pop_size) + "_start_" + str(gens) + "_gens_anthra.h5"
...
if __name__ == "__main__":
    ...
    df = pd.read_hdf(str(start_pop_size) + "_start_" + str(gens) + "_gens_anthra.h5")
